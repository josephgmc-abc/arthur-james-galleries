...
const artists = Array.from(new Set(dummyArtworks.map(a => a.artist))).sort();
  ...
  const processedArtworks = useMemo(() => {
    ...
    if (priceRange !== "all") {
      result = result.filter(a => {
        ...
        // Exclude "Price Upon Request" from numeric ranges, or include them?
        // Usually, price upon request is excluded from strict numeric filters, or we put them in "over-250k"
        if (val === Infinity) return priceRange === "over-250k"; // Assume upon request is high value
    ...
    return result;
  }, [selectedArtists, sortOption, priceRange]);
    ...
    <div className="flex flex-col w-full bg-beige min-h-screen">
      ...
      <section className="relative h-[65vh] w-full overflow-hidden flex flex-col justify-end pb-24 px-6 md:px-12 bg-navy">
        ...
        <div className="relative z-20 max-w-4xl">
          <span className="font-sans text-[10px] tracking-[0.4em] text-gold uppercase mb-6 block">The Collection</span>
      ...
      <section className="py-24 px-6 md:px-12">
        ...
        <div className="flex justify-between items-center mb-16 border-b-[1.5px] border-navy/10 pb-8">
          <span className="font-sans text-[10px] tracking-widest text-charcoal/50 uppercase">
        ...
        {processedArtworks.length === 0 && (
          <div className="w-full text-center py-32">
            <p className="font-sans text-charcoal/50 tracking-widest uppercase text-xs">No works found for this selection.</p>
            <button 
              onClick={() => {
                setSelectedArtists([]);
                setPriceRange("all");
          ...
          <>
            ...
            >
              ...
              <div className="p-8 border-t-[1.5px] border-white/10 bg-navy mt-auto">
                <button 
                  onClick={() => setIsFilterOpen(false)}
                  className="w-full bg-beige text-navy py-5 font-sans text-xs tracking-widest uppercase hover:bg-gold transition-colors duration-500"
                >
      ...
      {/* Basic custom scrollbar for the drawer */}
